...
exports.handleDeleteResource = async (req, res) => {
  ...
  try {
    ...
    // Step 2: Cancel all pending rental requests for this resource
    const [pendingRentals] = await db.promise().query(
      `SELECT rental_id, renter_id FROM rentals 
       WHERE resource_id = ? AND status = 'Pending'`,
      [resourceId]
    );

    if (pendingRentals.length > 0) {
      // Update all pending rentals to Rejected
      await db.promise().query(
        `UPDATE rentals SET status = 'Rejected' 
         WHERE resource_id = ? AND status = 'Pending'`,
        [resourceId]
      );

      // Notify each renter about the cancellation
      for (const rental of pendingRentals) {
        const message = `Your rental request for "${resourceName}" has been canceled because the resource was deleted by the owner.`;
        await db.promise().query(
          `INSERT INTO notifications (sender_id, receiver_id, rental_id, message) 
           VALUES (?, ?, ?, ?)`,
          [userId, rental.renter_id, rental.rental_id, message]
        );
      }
    }
